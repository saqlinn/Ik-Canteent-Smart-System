...
import { Bell, ShoppingCart, Search, Plus, Star, Clock, Sparkles, Home, History as HistoryIcon, LogOut, Receipt, ArrowLeft, Menu as MenuIcon, X } from "lucide-react";
...
type CategorySetting = {
  category: string; opening_time: string | null; closing_time: string | null;
};
...
function MenuPage() {
  ...
  useEffect(() => {
    // Subscribe to menu_items (removed orderBy to avoid composite index requirements)
    const menuQ = query(
      collection(db, "menu_items"),
      where("available", "==", true)
    );
    const unsubMenu = onSnapshot(menuQ, (snap) => {
      const fetchedItems = snap.docs.map((d) => ({ id: d.id, ...d.data() } as MenuItem));
      // Sort in-memory by created_at (ascending)
    ...
    // Subscribe to category_settings
    const unsubCats = onSnapshot(collection(db, "category_settings"), (snap) => {
      const map: Record<string, { open: string | null; close: string | null }> = {};
      snap.docs.forEach((d) => {
        const data = d.data() as CategorySetting;
        map[data.category] = { open: data.opening_time, close: data.closing_time };
      });
      setClosings(map);
    }, (err) => {
  ...
  const SidebarContent = () => (
    <>
      <Link to="/" onClick={() => setSidebarOpen(false)}><Logo /></Link>
      ...
      <nav className="mt-8 space-y-1">
        {[
          ...
          <Link key={label} to={to as any} onClick={() => setSidebarOpen(false)}
        ...
        {isAdmin && (
          <Link to="/admin" onClick={() => setSidebarOpen(false)} className="flex items-center gap-3 rounded-xl px-4 py-3 text-sm font-medium text-primary border border-primary/30">
  ...
  return (
    <div className="flex min-h-screen">
      ...
      {sidebarOpen && (
        <div className="fixed inset-0 z-50 md:hidden">
          ...
          <aside className="absolute left-0 top-0 flex h-full w-72 flex-col border-r border-border/50 bg-background p-6">
            <button onClick={() => setSidebarOpen(false)} className="absolute right-4 top-4 rounded-full p-2 text-muted-foreground hover:text-foreground">
              <X className="h-5 w-5" />
            </button>
            <SidebarContent />
          </aside>
      ...
      <main className="flex-1">
        <header className="flex items-center justify-between border-b border-border/50 px-4 py-4 md:px-8 md:py-6">
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2 md:hidden">
              <Button variant="ghost" size="icon" onClick={() => navigate({ to: "/" })} className="h-9 w-9 rounded-full bg-surface/50">
                <ArrowLeft className="h-5 w-5" />
              </Button>
              <button onClick={() => setSidebarOpen(true)} className="rounded-xl p-2 text-muted-foreground hover:bg-card hover:text-foreground">
                <MenuIcon className="h-5 w-5" />
              </button>
            </div>
            <div>
              <h1 className="font-display text-2xl font-bold md:text-3xl">Order Food 🍽️</h1>
              <p className="text-xs text-muted-foreground md:text-sm">{new Date().toLocaleDateString("en-IN", { weekday: "long", day: "numeric", month: "long" })}</p>
            </div>
          </div>
        ...
        <div className="px-4 py-4 md:px-8 md:py-6">
          ...
          <div className="mt-4 flex gap-2 overflow-x-auto pb-2 md:mt-6 md:flex-wrap md:gap-3">
            {CATS.map((c) => (
              <button key={c} onClick={() => setCat(c)}
                className={`shrink-0 rounded-full px-5 py-2 text-sm font-semibold transition md:px-6 md:py-2.5 ${
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2 md:mt-8 md:gap-6 xl:grid-cols-3">
              ...
              return (
                <article key={it.id} className="overflow-hidden rounded-2xl border border-border bg-card shadow-card">
                  <div className="relative">
                    <img src={img} alt={it.name} loading="lazy" width={800} height={800} className="h-48 w-full object-cover md:h-56" />
                    {it.best_seller && (
                  ...
                  <div className="p-4 md:p-5">
                    ...
                    <Button onClick={() => handleAdd(it)} className="mt-3 w-full bg-primary/15 text-primary hover:bg-primary/25 md:mt-4">
